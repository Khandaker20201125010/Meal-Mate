...
export async function middleware(req) {
  ...
  if (role === "admin") {
    // Admin routes
    if (
      (url.pathname === "/api/add-menu" && req.method === "POST") || // Admin can add place
      (url.pathname === "/api/manage-users" && req.method === "GET") || // Admin can manage users
      (url.pathname === "/api/manage-menu" && req.method === "GET") || // Admin can view all bookings
      (url.pathname === "/api/manage-bookings" && req.method === "GET") // Admin can view destinations
    ) {
      return NextResponse.next();
    }
  }

  // Customer-only routes (e.g., My Wishlist, My Bookings)
  if (role === "customer") {
    // Customer routes
    if (
      (url.pathname === "/api/my-orders" && req.method === "GET") || // Customer can view their wishlist
      (url.pathname === "/api/my-bookings" && req.method === "GET") || // Customer can view their bookings
      (url.pathname === "/api/payment-history" && req.method === "GET") // Customer can view all their bookings
    ) {
      return NextResponse.next();
    }
  }
  ...
  if (url.pathname.startsWith("/api/my-orders/") && req.method === "GET") {
